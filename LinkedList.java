...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		removeAll(first, el, 0);
	}

	private void removeAll(Node ref, E el, int currIndex) {
		if (ref == null) {
			return;
		} else {
			if (currIndex == 0 && el.equals(ref.data)) {
				first = ref.next;
				ref = first;
				currIndex = 0;
				n--;
	...
	private void duplicateAll(Node ref, E el) {
		if (ref == null) {
			...
			if (el.equals(ref.data)) {
				Node newNode = new Node(el, ref.next);
				ref.next = newNode;
				ref = newNode.next;
				n++;
			} else {
